...
| 1.4     | 2026/07/17 | Nguyen Minh Tri | 16章エラー要件にE011（重複操作エラー）を追加。6章REQ-016および`10_API設計.md`7章と整合させるため。 |
...
| REQ-027 | 在庫調整・注文ステータス変更・クーポン発行のいずれかが実行される | 当該操作が完了する | inventory_logsまたはアプリケーションログに実行者・操作内容・日時が記録される（`14_セキュリティ設計.md`14.1節） |
...
本システムはStripe PaymentIntentの自動キャプチャモード（`capture_method=automatic`）を採用する（`12_詳細設計書.md`5.5節）。そのため与信確保と売上確定はStripe側で1回のAPI往復に統合され、自社DBには`authorized`という中間状態を永続化しない。`authorized`はENUMの値としては残すが、これは将来的に不正利用対策として手動キャプチャ（与信確保後にAdminが確認してから確定）へ切り替える拡張余地として予約する（本スコープでは未使用）。
...
画面遷移・条件付き遷移は`05_画面遷移図.md`、各画面のレイアウト・表示項目は`06_画面設計.md`を参照。
...
7.4節（非機能要件 Security）を参照。加えて、決済関連の詳細は `14_セキュリティ設計.md` に記載する。
...
- 障害調査のための決済・在庫ログの参照手順（`20_運用保守手順書.md`）
...
`00_開発計画書.md` 2.2節、`01_企画書.md` 9.2節と同一。
...
本要件定義書は、EC Site の機能要件・非機能要件に加え、本プロジェクトの核心である「スナップショット」「在庫2段階制御」「税率」「クーポン」「決済」の業務ルール（9章）を明文化した。以降の `09_テーブル定義.md` `10_API設計.md` `12_詳細設計書.md` は、すべて本章のルールIDを根拠として設計する。
