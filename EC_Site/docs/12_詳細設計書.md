...
| 1.3 | 2026/07/22 | Nguyen Minh Tri | 設計監査で発見: 5.2節ステップ7が割引前subtotalに対して税額を計算していた（BR-TAX-005違反）。フェーズ分割（小計確定→割引の行別按分→按分後の税額計算）に修正し、09_テーブル定義のline_discount列と連動。5.2節ステップ6にcouponsロックの明記を追加（実装コード（guide 06_クーポン.html）は既にlockForUpdateしていたが本書の記述が追いついていなかったための同期）。5.2節ステップ13にStripe¥50下限バイパス分岐（BR-PAY-004）を追加、payments.bypass_reasonと連動。5.5節ステップ4-dと5.2節ステップ13で重複していた在庫確定ロジックを5.5.1節 InventoryService::commitReservedForOrder に共通化。 |
...
本書は11_基本設計書の外部設計を、実装可能な単位（Controller/Service/Model/FormRequest/Exception）まで分解する。特に5章（Service詳細設計）は、コーディング時に最も参照頻度の高いセクションである。
...
6. coupon_code が指定されていれば CouponService::validate を呼び出す（E005ならロールバック）。
   coupons行は SELECT ... FOR UPDATE でロックしてから検証する（4のinventoriesロックと同じ
   トランザクション内で行い、used_count/usage_limitの同時注文による二重利用を防ぐ）
7. 金額を計算する。割引額は subtotal が確定するまで計算できず、かつ消費税は割引後の実売上額に
   対して計算する必要がある（BR-TAX-005）ため、2フェーズで行う:
     フェーズ1（各行の小計とsubtotalを確定）:
     ...
     discount_total = coupon ? CouponService::calculateDiscount(subtotal, coupon) : 0  # BR-CPN-002
     フェーズ2（discount_totalを各行へline_subtotal比で按分してから税額を計算、BR-TAX-005）:
       discount_total == 0 の場合: 各行 line_discount = 0
       discount_total > 0 の場合:
         最終行以外の各行: line_discount = floor(line_subtotal * discount_total / subtotal)
         最終行:          line_discount = discount_total − Σ(他行のline_discount)
                          （端数（丸め誤差）を最終行に寄せ、Σ line_discount = discount_total を保証する）
       各行: line_tax   = floor((line_subtotal − line_discount) * tax_rate)  # BR-TAX-004
...
9. cart_items の各行を order_items へ変換して作成する。
   product_name / variant_label / sku / unit_price / tax_rate / line_subtotal / line_discount / line_tax / line_total を
   7で計算した値でコピーする（BR-ORD-002）
10. 各 variant について在庫を確保する:
    ...
    （実際の消費確定はWebhook受信時、または13の決済バイパス時。ここでは仮押さえのみ）
11. coupon が適用されていれば coupons.used_count += 1（BR-CPN-003）
12. cart.status を converted に更新する（新しいactiveカートは次回追加時に自動作成、5.1節）
13. 決済を作成する。Stripeの JPY 課金下限（¥50未満のPaymentIntentはStripe API自体がエラーを返す）に
    対応するため、grand_total に応じて分岐する（BR-PAY-004）:
      a. grand_total == 0 の場合:
           Stripeを呼ばず、payments を status=captured, bypass_reason='zero_amount', paid_at=now() で作成する
      b. 0 < grand_total < 50 の場合:
           同上の手順で bypass_reason='below_minimum' として作成する
      c. grand_total >= 50 の場合（通常フロー）:
           Stripe PaymentIntent を amount=grand_total で作成し、
           payments を status=pending, bypass_reason=NULL で作成する（5.5節のWebhookで確定させる）
    a/bでバイパスした場合は、この場で InventoryService::commitReservedForOrder(order) を呼び出し
    在庫引き当てを確定する（5.5節ステップ4-dと同一ロジックを共有、二重実装しない）。
    discount_total は7で計算した実際のクーポン割引額のまま使用し、
    grand_total を¥0へ到達させるための人為的な水増しは行わない（BR-PAY-004、会計データの正確性のため）
14. a/bでバイパスした場合、orders.status を pending → paid に更新する（Webhookを待たずこの場で確定）
15. トランザクションコミット
戻り値: order（id, order_number, 金額内訳, stripe_client_secret ※バイパス時はnull）
```
...
4. event.type == 'payment_intent.succeeded' の場合:
   ...
   d. InventoryService::commitReservedForOrder(order) を呼び出し、在庫引き当てを確定する
      （5.5.1節。5.2節ステップ13のバイパス経路と同一ロジックを共有、BR-INV-004）
   e. トランザクションコミット
...
## 5.5.1 InventoryService::commitReservedForOrder（在庫引き当て確定の共通化、BR-INV-004）

決済成功時の在庫確定処理は、5.5節（Webhook経由の通常決済）と5.2節ステップ13（¥0/¥50未満のバイパス決済）の**2箇所から呼ばれる**。同じロジックを2箇所に重複実装しないため、InventoryServiceの共通メソッドとして抽出する。

```text
入力: order
1. 対象 order の各 order_items について:
     inventories.quantity_reserved -= quantity   # 確定消費、availableは5.2節ステップ10で既に減算済みなので変更しない
     inventory_logs に change_type=purchase_deduct, reference_type=orders, reference_id=order.id で記録
戻り値: なし（呼び出し元のトランザクション内で実行される前提。このメソッド自体はトランザクションを開始しない）
```
...
```text
入力: order_id
1. payments を order_id かつ status=captured で検索、存在しなければ E007（返金対象の決済データが見つからない。状態不整合ではなくデータ未検出のため10_API設計7章のE006/E007使い分けに従いE007とする）
...
| 処理 | トランザクション範囲 | 備考 |
| --- | --- | --- |
| OrderService::place | orders + order_items + inventories + coupons + payments（作成のみ） | 在庫ロック順序を`variant_id`昇順に固定しデッドロックを回避。¥0/¥50未満バイパス時（5.2節ステップ13a/b）は同一トランザクション内でInventoryService::commitReservedForOrderとorders.status=paidまで完結させる |
...
| InventoryService::commitReservedForOrder | inventories + inventory_logs | 独自のトランザクションを持たず、呼び出し元（OrderService::place または PaymentWebhookService）のトランザクション内で実行される（5.5.1節） |
...
Webhook処理内部で発生したその他の例外（DB接続断等）は、Stripeへは200を返しつつ内部ログに記録する方針とする（9章）。これはStripe Webhookの「失敗時は自動リトライされる」仕組みに反して200を返す一見矛盾した設計だが、リトライに任せると同一イベントが際限なく再送され続けるリスクがあるため、恒久的な内部エラー（バグ等）は運用側の手動対応（20_運用保守手順書）に委ねる判断とした。
...
本書はController/Service設計をAPI-ID単位で記述しているため（4章・5章）、07_機能一覧のFUNC-IDや02_要件定義書のREQ-IDから本書の該当箇所を探す際は、10_API設計5章のAPI-ID列を経由する。下表は4章のController表にFUNC-ID/REQ-IDを付加したもので、この逆引きを1ステップで完結させる。
...
| WebhookController | API-017 | FUNC-013 / 017 / 018 | -（Systemアクター起点のためREQ紐付けなし。03_ユースケースUC-018参照） |
...
10_API設計のAPI-ID → 本書のService/Controller → 15_単体試験仕様書のUT-ID/16_結合試験仕様書のIT-IDの順に一意に追跡できる。特にOrderService::place（5.2節）とPaymentWebhookService::handleStripeEvent（5.5節）は、UT-ORD系・IT-PAY系の主要試験対象である。

---

# 15. まとめ

本書の中心はOrderService::place（5.2節）とPaymentWebhookService::handleStripeEvent（5.5節）である。この2メソッドを正確に実装できれば、本プロジェクトの学習目標（トランザクション・在庫・決済）の大部分を達成したことになる。実装時は必ず本章の疑似コードとBR-ID（02_要件定義書9章）を突き合わせながら進めること。
