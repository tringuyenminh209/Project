...
| 1.1 | 2026/07/21 | Nguyen Minh Tri | 7.4節にローカル開発環境（Docker Desktop）の自動起動について追記。restart:unless-stopped単体では不十分で、Docker Desktop自体の「サインイン時に自動起動」設定が別途必要な点を明記（guide 01章のdocker-compose.ymlにrestart policy追加と対応）。 |
...
| 項目 | 方針 |
| --- | --- |
| Restart Policy | `unless-stopped`（Docker daemon再起動時に自動再開。手動`stop`した場合は再開しない） |
| ローカル開発環境の自動起動 | `unless-stopped`はDocker daemonが起動していないと効果がない。Windows/MacではDocker Desktop自体がdaemonを兼ねるため、OS起動時に自動でコンテナを立ち上げたい場合はDocker Desktop側の設定（Settings → General → "Start Docker Desktop when you sign in"）を別途有効化する必要がある（リポジトリのファイルでは制御不可、手動設定手順としてguide 01章に記載） |
