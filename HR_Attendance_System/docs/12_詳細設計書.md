...
| 1.4 | 2026/07/21 | Nguyen Minh Tri | 自動セキュリティレビューで発見: 6章Employee行の`fillable（主要）`が`role_id`/`status`/`password_hash`等の権限関連列を含んだままだった（Mass Assignment / Privilege Escalationのリスク、14_セキュリティ設計 v1.1で方針確定済み）。実際の`$fillable`は`employee_id`/`name`/`email`のみに限定し、権限関連列はEmployeeServiceが`forceFill()`/`forceCreate()`で明示的に設定する設計に統一。 |
| 1.5 | 2026/07/21 | Nguyen Minh Tri | 11章の例外マッピング表にLoginFailedException（E001/401、AuthService::login）の行が抜けていた（E002〜E010は網羅済みだったがE001が唯一未記載）。行を追加し、独自例外は各クラス自身の`render()`で自己変換する実装（Handler.php集中変換ではない）に合わせて説明文も訂正。 |
...
| --- | --- | --- | --- |
| Employee | employee_id, name, email（他の一般プロフィール列のみ） | belongsTo Role/Department/Shift、hasMany AttendanceRecord/LeaveRequest/AuditLog | password_hashはhidden属性としAPIレスポンスから除外する。**role_id/department_id/shift_id/status/password_hashは意図的に`$fillable`から除外**（Mass Assignment / Privilege Escalation対策、14_セキュリティ設計 8.1）— EmployeeServiceがrole:admin Middleware通過済みの箇所でのみ`forceFill()`/`forceCreate()`により明示的に設定する |
...
| --- | --- | --- | --- |
| LoginFailedException（独自） | AuthService::login（email未存在/status=inactive/password不一致の3パターン共通、14_セキュリティ設計 14章のenumeration対策） | E001 | 401 |
...
独自例外（LoginFailed/Duplicate/CheckOutWithoutCheckIn/LeaveRequestAlreadyProcessed/CsvExport）は`app/Exceptions/`配下に定義し、各クラスが`render()`メソッドでError ID・HTTP Statusへ自己変換する（`Handler.php`側で個別にcatchしない — Laravelの規約: `render()`を持つ例外は自動的にそれが使われる）。これによりController/Serviceでは業務条件のみを判定し、HTTP Statusの詳細を意識しない。
